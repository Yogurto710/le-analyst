"""legacy_prompts.py — archived system prompts for backward-compatibility commands.

This module exists so the ~50KB legacy initiate prompt doesn't sit inline in
analyst.py, where it pads every git diff, fills the CLAUDE.md cognitive
context for new sessions, and obscures the active prompts that drive the
current product.

Imported only by the `initiate_legacy` command in analyst.py. The contract
is the same as the constant always had: a string with a single `{today}`
formatting placeholder.

Deliberate departure from CLAUDE.md's "stay single-file until it actively
hurts" rule — this constant *is* the case where single-file actively hurts
(see git log for analyst.py around 2026-06-17 for the audit context that
prompted the split). The active product (research / initiate / mini-app /
all validators) stays in analyst.py.
"""
