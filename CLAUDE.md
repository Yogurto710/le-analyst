...
4. **Review + revise** (Phase 4, optional): code-side checks C1-C13 parse the draft. If any fire, the model gets the finding list and produces a targeted revision; we save the revised version. Zero added cost when no findings; ~$0.20 + 3-5 min when a revision runs.
...
- **C11** Peer Comp Table column count (new shape only): table must have all 10 required columns (Company | EV | Rev | 2-yr Rev CAGR | EV/Rev × 3 | P/E × 3). Catches MU 2026-06-16 v0.2 regression where the model collapsed forwards into a single "Fwd P/E" column, shipping a 7-column table.
- **C12** LTM cross-check (new shape only): for each row in Financial Summary, flag if LTM > 2.5× LFY — a >2.5x jump in 1-2 quarters of TTM roll usually means a single recent quarter was annualized instead of summing the last 4 disclosed quarters. Caught MU 2026-06-16 v0.2 where LTM Net Income $32B was 3.8× FY25 LFY $8.5B (likely Q2 FY26 NI × 4).
- **C13** 2-yr CAGR validation (new shape only): recompute `(FY+2E rev / LFY rev)^(1/2) - 1` from the Street consensus + Financial Summary tables; compare to the stated CAGR in the Business snapshot table; flag if diff > 3pp. Caught MU 2026-06-16 v0.2 where stated CAGR was 10.6% but recomputed from LFY $37.4B → FY+2E $112B was 73.1% (62pp gap).

The legacy 11-section `initiate_legacy` command runs only C1-C7 (those were tuned to the legacy layout). The new 8-section `initiate` runs C5+C7 PLUS C8-C13, which parse the new Financial Summary / Peer Comp Table layout inside Appendix at H3. C5, _find_section, and _extract_open_questions all accept the locked Chinese section aliases (see `_SECTION_NAMES_ZH` in analyst.py) — a Chinese-output report with `## 投资论点` / `## 财务摘要` etc. is validated without a separate zh validator path.
...
- **`--lang zh`**: the per-command template plus the Chinese instruction is appended. The instruction is split into TWO blocks (constant-pair, not one block — this matters):
  - `_LANG_INSTRUCTION_ZH_BASE` — language + preserve-verbatim rules (numbers, currency `$`, source URLs byte-identical, tickers in Latin, English source titles untranslated) + Chinese financial-term glossary (EBITDA/FCF/etc. as-is; established Chinese names like 网易/美光/美满电子) + universal Investment-Framework discipline (no 买入/卖出/持有, no 价格目标 — these phrasings are very natural in Chinese, so the rule must be explicit there). Safe to append to ANY command's prompt — carries no section lockdown.
  - `_LANG_INSTRUCTION_ZH_INITIATE` — locks the H1 title to `投资简报` ("investment brief"; the term retail Chinese readers use) and the H2 section headers to fixed Chinese strings (`Thesis → 投资论点`, `Business snapshot → 业务概览`, `Bull/Bear/Base → 看多/看空/基准情景`, etc. — see `_SECTION_NAMES_ZH` for the canonical map). Downstream validators (C5, `_find_section`, `_extract_open_questions`) key on these exact strings via `_SECTION_NAMES_ZH`, which is why the lockdown is necessary for initiate. **This block must NOT be appended to `research` or `initiate_legacy`** — doing so silently produced initiate-shaped Chinese research briefs through 2026-06-24 (caught on RBLX), because the section-header lockdown is far more forceful than the per-command template and overrides it.
  - `_lang_instruction(lang, include_initiate_sections=False)` returns BASE only; pass `include_initiate_sections=True` (only `initiate` does this) to get BASE + INITIATE.
